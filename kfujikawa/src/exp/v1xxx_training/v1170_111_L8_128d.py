...
class HistoryImpressionFeatureExtractor(FeatureExtractor):
    ...
    def __call__(
        ...
        outputs = {}
        outputs["num_history_articles"] = np.log1p(len(users.impression_ts[user_index]))
        ...
        outputs["elapsed_ts_from_history"] = np.clip(
            np.log1p((imp_ts_inview - users.impression_ts[user_index].max()) / 60**2),
            0,
            5,
        )
        outputs["history_read_time_mean"] = users.read_time[user_index].mean()
        outputs["history_scroll_percentage_mean"] = users.scroll_percentage[
            user_index
        ].mean()
        outputs["history_scroll_zero_ratio"] = (
            users.scroll_percentage[user_index] == 0
        ).mean()
...
class HistoryArticleFeatureExtractor(FeatureExtractor):
    ...
    def __call__(
        ...
        history_article_indices = users.article_indices[user_index]
        history_unique_articles = compute_unique_features(
            history_article_indices,
            ranks=np.arange(len(history_article_indices)) + 1,
            imp_ts=users.impression_ts[user_index],
            readtime=users.read_time[user_index],
            scroll_percentage=users.scroll_percentage[user_index],
        )
